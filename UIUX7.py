...
from huggingface_hub import hf_hub_download  # Ensure this import is added
...
def initialize_model(model_name, model_type):
    ...
    if model_type in [Model_Type.gptq, Model_Type.ggml]:
        # Assuming model_name is a valid repo_id on Hugging Face Hub
        model_path = hf_hub_download(repo_id=model_name, filename="pytorch_model.bin")
    else:
        # For full_precision or other types that use local models
        model_path = os.path.join(models_folder, model_name)
    aglm_model = LlamaModel(model_path)
